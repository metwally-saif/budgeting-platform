...
const { Identitytoolkit } = identitytoolkit_v3;
export const { relyingparty } = new Identitytoolkit({ auth });
...
export async function getIdToken(req: Request) {
  try {
    ...
    let result: DecodedIdToken | null = null;
    ...
    if (idToken) {
      ...
      if (token) {
        ...
        result = token as DecodedIdToken;
      }
    }
